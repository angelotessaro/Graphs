...
void remove_topologica(lin_lista *ord_topologica, int vertice){
	for(elemento *a = ord_topologica->inicio; a!= NULL; a = a->proximo){
		elemento *b = (elemento *) malloc(sizeof(elemento));  
		b = a->proximo;
		elemento *aux = a;
		if(a->v == vertice){
			/*if(a == ord_topologica->inicio){
				ord_topologica->inicio = b;
				free(a);
				printf("ordenacao topologica: ");
				for(b = ord_topologica->inicio; b!=NULL; b = b->proximo){
					printf("%d, ", b->v);
				}
				break;
			}*/
